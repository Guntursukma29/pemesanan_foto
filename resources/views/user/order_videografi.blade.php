...
@section('content')
    <div class="container mt-4">
        <form action="{{ route('pemesanans.videografi.store') }}" method="POST">
            @csrf
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr>
                                <td>{{ $videografi->nama }}</td>
                                <td id="harga">Rp {{ number_format($videografi->harga_special, 0, ',', '.') }}</td>
                                <td id="tenaga_kerja">{{ $videografi->tenaga_kerja_spesial }}</td>
                                <td id="waktu">{{ $videografi->waktu_spesial }}</td>
                                <td id="penyimpanan">{{ $videografi->penyimpanan_special }}</td>
                            </tr>
                        </tbody>
                    </table>
                    <!-- Hidden field for package ID -->
                    <input type="hidden" name="id_paket" value="{{ $videografi->id }}">
    ...
    <script>
        ...
        const paketData = {
            special: {
                harga: 'Rp {{ number_format($videografi->harga_special, 0, ',', '.') }}',
                tenagaKerja: '{{ $videografi->tenaga_kerja_spesial }}',
                waktu: '{{ $videografi->waktu_spesial }}',
                penyimpanan: '{{ $videografi->penyimpanan_special }}'
            },
            platinum: {
                harga: 'Rp {{ number_format($videografi->harga_platinum, 0, ',', '.') }}',
                tenagaKerja: '{{ $videografi->tenaga_kerja_platinum }}',
                waktu: '{{ $videografi->waktu_platinum }}',
                penyimpanan: '{{ $videografi->penyimpanan_platinum }}'
            }
        };
